...
def process_audio(audio_path):
    try:
        ...
        transcription = model(audio_data, return_timestamps=True)["text"]
...
def collect_audio():
    ...
    try:
        ...
        # Clean the transcript, message, and details to remove characters that might cause encoding issues
        def clean_string(s):
            if isinstance(s, str):
                return re.sub(r'[^\x00-\x7F]+', '', s).replace('\n', ' ').replace('\r', '')
            return ""

        cleaned_transcript = clean_string(result['transcript'])
        cleaned_message = clean_string(result['message'])
        cleaned_details = clean_string(result['details'])
        
        response.headers.add('transcript', cleaned_transcript)
        response.headers.add('message', cleaned_message)
        response.headers.add('details', cleaned_details)
        return response
    except Exception as e:
